...
(function(){
  ...
  const sameProjectBeforeStart = JSON.stringify(S.analyses);
  const sameProjectRunBefore = JSON.stringify(findCleanupWorkbenchRun(run.id));
  saveCalls = 0;
  ui.cleanupReview.workbenchStatusFilter = 'deferred';
  startCleanupReview('p1', [], '{}', 'import-cleanup');
  assert.strictEqual(ui.cleanupReview.activeRunId, '');
  assert.strictEqual(ui.cleanupReview.workbenchStatusFilter, 'all');
  assert.strictEqual(getActiveCleanupWorkbenchRun(), null);
  assert.strictEqual(isActiveCleanupWorkbenchRunReview(), false);
  assert.strictEqual(JSON.stringify(S.analyses), sameProjectBeforeStart);
  assert.strictEqual(JSON.stringify(findCleanupWorkbenchRun(run.id)), sameProjectRunBefore);
  assert.strictEqual(saveCalls, 0);
  const freshSameProjectHtml = renderCleanupReviewSection();
  assert(freshSameProjectHtml.includes('Cleanup-Vorschläge laden'), 'fresh review import workflow remains visible');
  assert(!freshSameProjectHtml.includes('Persistenter Workbench-Run'));
  assert(!freshSameProjectHtml.includes('data-cleanup-workbench-case-status'));
  assert(!freshSameProjectHtml.includes('Run abschließen'));
  assert(!freshSameProjectHtml.includes('Run wieder öffnen'));
  openCleanupWorkbenchRun(run.id);
  const otherProjectBeforeStart = JSON.stringify(S.analyses);
  saveCalls = 0;
  startCleanupReview('p2', [], '{}', 'import-cleanup');
  assert.strictEqual(getActiveCleanupWorkbenchRun(), null);
  assert.strictEqual(isActiveCleanupWorkbenchRunReview(), false);
  assert.strictEqual(JSON.stringify(S.analyses), otherProjectBeforeStart);
  const freshOtherProjectHtml = renderCleanupReviewSection();
  assert(!freshOtherProjectHtml.includes('Persistenter Workbench-Run'));
  assert(!freshOtherProjectHtml.includes('Run abschließen'));
  assert(!freshOtherProjectHtml.includes('Run wieder öffnen'));
  assert.strictEqual(saveCalls, 0);
  ui.cleanupReview.activeRunId = run.id;
  ui.cleanupReview.projectId = 'p2';
  const mismatchBefore = JSON.stringify(S.analyses);
  assert.strictEqual(getActiveCleanupWorkbenchRun(), null);
  assert.strictEqual(isActiveCleanupWorkbenchRunReview(), false);
  assert.strictEqual(JSON.stringify(S.analyses), mismatchBefore);
  clearCleanupReview();
  assert.strictEqual(ui.cleanupReview.activeRunId, '');
  assert.strictEqual(ui.cleanupReview.workbenchStatusFilter, 'all');
  assert.strictEqual(ui.cleanupReview.visible, false);
  ui.cleanupReview.projectId = 'p2';
  ui.cleanupReview.mode = 'csv-comparison-transform';
  const oldRunBeforeNewBinding = JSON.stringify(findCleanupWorkbenchRun(run.id));
  const newRun = createCleanupWorkbenchRunFromReview({ summary: 'new' }, [{ type: 'match-existing', targetFeatureId: 'f2', resolvedTitle: 'New Run', confidence: 'hoch' }], []);
  hydrateCleanupReviewFromWorkbenchRun(newRun);
  assert.strictEqual(ui.cleanupReview.activeRunId, newRun.id);
  assert.strictEqual(getActiveCleanupWorkbenchRun().id, newRun.id);
  assert.strictEqual(JSON.stringify(findCleanupWorkbenchRun(run.id)), oldRunBeforeNewBinding);
  hydrateCleanupReviewFromWorkbenchRun(run);
  ...
  const renderSingleLegacy = (proposal) => {
    ui.cleanupReview.activeRunId = '';
    ui.cleanupReview.visible = true;
    ui.cleanupReview.mode = 'csv-comparison-transform';
    ui.cleanupReview.summary = '';
    ui.cleanupReview.openQuestions = [];
    ui.cleanupReview.proposals = [Object.assign({ _idx: 0, type: 'match-existing', targetFeatureId: 'f1', resolvedTitle: 'Legacy Karte', confidence: 'hoch', _done: false, _ignored: false, _queuedMainChat: false, _queuedDedupe: false, _reviewedOnly: false }, proposal || {})];
    return renderCleanupReviewSection();
  };
  assert(renderSingleLegacy({ _done: true }).includes('<span class="pill pill-success">übernommen</span>'));
  const ignoredLegacy = renderSingleLegacy({ _done: true, _ignored: true });
  assert(ignoredLegacy.includes('<span class="pill pill-outline">ignoriert</span>'));
  assert(!ignoredLegacy.includes('<span class="pill pill-success">übernommen</span>'));
  assert(renderSingleLegacy({ _done: true, _reviewedOnly: true }).includes('<span class="pill pill-info">geprüft</span>'));
  assert(renderSingleLegacy({ _done: true, _queuedMainChat: true }).includes('<span class="pill pill-info">für Hauptchat vorgemerkt</span>'));
  assert(renderSingleLegacy({ _done: true, _queuedDedupe: true }).includes('<span class="pill pill-info">für Dedupe vorgemerkt</span>'));
  assert(!renderSingleLegacy({ _done: false }).includes('<span class="pill pill-success">übernommen</span>'));
  reopened.reviewState.cases[ids[0]].status = 'reviewed';
  upsertCleanupWorkbenchRun(reopened);
  hydrateCleanupReviewFromWorkbenchRun(reopened);
  ui.cleanupReview.proposals[0]._done = true;
  ui.cleanupReview.proposals[0]._ignored = true;
  ...
  assert(!workbenchHtml.includes('für Dedupe vorgemerkt'));
})();
`;
...
const context = {
  ...
  window: {}, navigator: { clipboard: { writeText(){ return Promise.resolve(); } } }, Blob: function(){}, URL: { createObjectURL(){return ''}, revokeObjectURL(){} },
