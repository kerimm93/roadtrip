...
- Dedupe ist Preview-only: keine Merge-, Archivierungs-, Lösch-, Status-, Pool-
  oder Duplicate-Markierungs-Mutation.
...
- Dedupe-Entscheidungen dürfen nur über `roadtrip-dedupe-decisions-v1` in die
  lokale Preview; A/B-IDs und `canonicalFeatureId`/`duplicateFeatureId` sind auf
  das lokale Paar begrenzt.
...
Commitpfad übergeben. Sie ist geplant, nicht implementiert.
