...
## Cleanup-Workbench Bulk-Review und Status-/Pool-Mutationsklasse

Phase A nutzt eine von Apply getrennte UI-Auswahl (`workbenchSelectedCaseIds`). Der gemeinsame Einzel-/Bulkpfad validiert den vollständigen Batch live, baut eine State-Kopie, erlaubt nur Case-`status`/`updatedAt` und Run-`updatedAt`, prüft den restlichen State strukturell, speichert genau einmal asynchron und restauriert bei Fehlern den vollständigen vorherigen State. Completed Runs sowie valide angewendete und beschädigte Application-Cases bleiben read-only.

Cases besitzen additiv `statusPoolDecision` (ältere Records: `null`). Der lokale Shape bindet `roadtrip-status-pool-decisions-v1`, Projekt, Run, Case, Feature, exakten Ausgangs- und Zielzustand und `validatedAt`. Frisches Cleanup-Modell-JSON kann dieses lokale Feld nicht setzen. `mainChatApplication` besitzt additiv `applicationType`; Altanwendungen mit ausschließlich den bisherigen Update-Feldern werden als `update-existing` gelesen, unbekannte Typen oder Feldmischungen als invalid.

Phase B erweitert den vorhandenen atomaren Commitkern über `mutationClass`, nicht über eine zweite allgemeine Engine. `status-pool-transition` friert Run-, Case-, Decision-, Application- und Featurebaselines ein, prüft Drift und verwendet denselben Candidate-State-/Protected-State-/Single-save-/Rollback-Ablauf. Am Feature sind ausschließlich `pool`, `status`, `updatedAt` erlaubt; am Case ausschließlich der typisierte Application-Nachweis und `updatedAt`, am Run ausschließlich `updatedAt`. `reviewStatus`, Trello, Inhalte und alle fremden Records bleiben geschützt.

Die neuen Felder liegen weiterhin im Whole-Record `S.analyses` und werden deshalb durch bestehenden JSON-/ZIP-Vollstate-Transport und Whole-Record-Sync lossless mitgeführt. Gist-/Merge-, Tombstone- und Persistenzarchitektur bleiben unverändert; das bekannte Mehrgeräte-Whole-Record-Risiko bleibt Nicht-Scope.
