...
- **Nächste Aktion:** Pro Fall muss erkennbar sein, ob eine menschliche Entscheidung,
  ein Hauptchat-Abgleich, ein strukturierter Dedupe-Review, ein Browser-Test oder ein Commit-Diff
  als nächstes ansteht.
- **Preview versus echte Mutation:** Preview-Ergebnisse, strukturierte Dedupe-Preview-Entscheidungen und
...
- **Strukturierte Dedupe-Preview ohne versteckte Apply-Aktion:**
  `roadtrip-dedupe-decisions-v1` darf Entscheidungen sichtbar machen, aber keine
  implizite Merge-/Archivierungs-/Löschaktion anbieten. Der bereits bestehende
  direkte Cleanup-Review-Papierkorbpfad für konkret identifizierte Dubletten bleibt
  visuell als bestätigungspflichtiger Sonderpfad getrennt; im CSV-Transformationsmodus
  bleibt diese direkte Dublettenaktion Review-only.
...
Preview-, Hauptchat-, strukturierte Dedupe- und Confirm-Pfade sowie den direkten Cleanup-Papierkorbpfad differenzierter abbildet. Alte
