...
- Nachzieh-Ausgangspunkt für den letzten Reviewbefund: Commit `53da9c01fc66a0b8de2f72ae8df6d74e779e2f32`.
...
- Reviewbefund zur ungültigen Kategorie bestätigt: Workbench-Eligibility darf eine `proposedCategory` außerhalb `FEATURE_CATEGORIES` nicht als anwendbar anzeigen, auch wenn gleichzeitig ein gültiger Titel-/Beschreibungswechsel vorliegt.
- Geschlossene Kategorieprüfung ergänzt: leere oder unveränderte `proposedCategory` erzeugt keine Änderung; echte Kategorieänderungen müssen exakt in `FEATURE_CATEGORIES` liegen, sonst werden Import/Eligibility/Batch/Commit für den ganzen Case blockiert. Es gibt keine Ersetzung durch `Sonstiges` und keine Teilanwendung.
- Nach erfolgreichem Workbench-Commit wird die Preview ohne zweiten Save aus dem persistierten Live-Run neu aufgebaut; angewendete Cases zeigen `mainChatApplication.appliedAt`, nicht ausgewählte anwendbare Cases bleiben aus Persistenz ableitbar.
- Workbench-Reset leert nur die Textarea und baut die persistierte Preview ohne Save neu auf; Legacy-Reset setzt die Preview weiterhin auf `null`.
...
- Ergänzte Nachziehtests: ungültige Kategorie allein und mit Titeländerung blockiert Eligibility/Selectability, ungültiger Kategorie-Import wird geschlossen abgewiesen, gefälschte Preview-/PendingBatch-Kategorie wird in Batch/Commit blockiert, gültige Kategorie bleibt commitfähig, erfolgreiche Workbench-Confirm-Preview wird aus Persistenz neu aufgebaut, `appliedAt` wird angezeigt, kein zweiter `saveAsync()` entsteht, nicht ausgewählter Case bleibt anwendbar, Workbench-Reset baut persistierte Preview neu auf und Legacy-Reset bleibt unverändert. Zusätzlich weiterhin UI-Modi für aktive/abgeschlossene Workbench- und Legacy-CSV-Pfade, Workbench-/Legacy-Promptwortlaut, strikte Decision/Application-Rohdatentypprüfung, malformed `splitProposal`, Ziel-Run-`result`-/`runStatus`-/fremder-Case-Protected-State-Regressionen sowie Rollbacktests für Entscheidungsimport- und Featurecommit-Fehler in `tests/cleanup-workbench-update-existing-bridge.test.js` ausgeführt und PASS.
