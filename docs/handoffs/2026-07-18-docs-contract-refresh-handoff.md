...
- initialer Docs-Commit: `65c2e71`
- finale Reviewkorrektur: im PR-/Abschlussbericht ausgewiesen
- Commit-Message: `docs: refresh core Roadtrip contracts and document 2026-07-18 cleanup realtest`
...
- Analyse-/Cleanup-/Review-Verträge wurden dokumentiert: untrusted model input,
  ...
  `roadtrip-dedupe-decisions-v1`, strukturierte Preview-only-Dedupe, erlaubte
  Teil-/Leerrückgaben, direkter Cleanup-Papierkorbpfad und enger
...
## Reviewkorrektur vor Merge

Vor dem Merge wurden zwei P2-Reviewbefunde eingearbeitet:

- Preview-only wurde auf die strukturierte Dedupe-Rückgabe
  `roadtrip-dedupe-decisions-v1` eingegrenzt. Der bestehende direkte
  Cleanup-Review-Papierkorbpfad für konkret identifizierte Dubletten wurde als
  bestätigungspflichtiger Sonderpfad dokumentiert; im CSV-Transformationsmodus
  bleibt diese direkte Aktion Review-only.
- Hauptchat-/Dedupe-Rückgaben wurden als zulässige Teilantworten dokumentiert:
  leere `decisions`- und `dedupeDecisions`-Arrays sind gültig und mutationsfrei;
  zurückgegebene IDs müssen bekannt, lokal konsistent und innerhalb derselben
  Antwort eindeutig sein.
...
- Kein strukturierter Dedupe-Apply implementiert und kein bestehender direkter
  Cleanup-Papierkorbpfad verändert.
